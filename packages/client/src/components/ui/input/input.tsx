...
export const Input = ({ style, customClass, placeholder, type }: CompontsProps) => {
  const [showPassword, setShowPassword] = useState(false);
  const isPassword = type === "password";

  return (
    <div className={ `${styles.box} ${customClass}`}>
      <input type={isPassword && showPassword? 'text' : type} placeholder={placeholder} className={`${styles.input}`} style={style} />
      {
